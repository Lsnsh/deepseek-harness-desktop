...
- **助手推理（"Think"/Deep diving）流式输出期间的超高能耗**
  （Activity Monitor 能耗影响 ≈ 1000，WebContent 主线程被打满）。内置 web
  GUI 的 "Think" 行会在流式输出时保持一条横向滚动的"最新一行"摘要：每次
  流式文本分片都会重新触发 `requestAnimationFrame` 链，其更新逻辑读取
  `element.scrollWidth`（强制一次全量同步布局），随后写入
  `element.scrollLeft`（强制一次全量合成 pass，并连带每帧重算 event
  regions）。会话很大时，整个推理阶段 WKWebView 主线程被锁定在 ~100%。
  现在装配后的运行时对 GUI 产物携带两处外科手术式补丁（由
  `scripts/patch-runtime.mjs` 在运行时装配阶段自动应用）：
  - "Think" 行摘要改用钳位哨兵值（`scrollLeft = 1e9`）贴右，不再计算
    `scrollWidth - clientWidth`，因此不再产生同步布局读取；跟随滚动每行
    限频到约 500 ms（30 帧），而不是每个分片都重新武装；
  - 会话帧刷新通知器增加 32 ms 刷新下限（尾沿重新武装、不丢事件），高
    频流式时重渲染 + 重绘从每帧一次降到约每秒 31 次。
  实测验证：流式期间滚动写入下降约 60%，零控制台错误，流式交付不变；
  哨兵值钳位后与原来的终点位置完全一致。两处上游代码片段在 0.1.1-rc.2 中
  均未变化，补丁对本版本的 bundle 同样生效。
